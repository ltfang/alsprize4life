...
GetVitalFeatures <- function(raw.data.name="train") {
	...
	rda.filename = paste("Data/vital_",raw.data.name,"_features.rda",sep="")
	if (file.exists(rda.filename)) {
		print(paste("loading saved vital features from",rda.filename,sep=" "))
		load(rda.filename)
		print("finished loading")
		return(vital.features)
	}

	# Get list of datasets in vital data
	vital.data<-GetVitalData(raw.data.name)
	columns<-names(vital.data)
	
	# Get a list of datasets containing feature data for each test
	feature.list<-mapply(GetDefaultFeatures,raw.data.name,vital.data,columns,SIMPLIFY=FALSE)

	vital.features<-Reduce(function(x,y) {merge(x,y,by="subject.id",all.x=TRUE)}, feature.list)
		
	# Save features data to RDA file
	save(vital.features, file = rda.filename)
