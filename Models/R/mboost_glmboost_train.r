...
MboostGlmboostTrain <- function(train.name="train",target=NULL,args=list(test.name=NULL,R=1,pls=FALSE,
	prune=FALSE,std=FALSE,features=c("static","alsfrs","fvc","svc","vital"), 
	slope.features=NULL)) {
	# Trains a MboostGlmboost model given training data.
	# Args:
	...
	if(FALSE) {
		features = c("static","alsfrs","fvc","svc","vital")
	   	...
	   	args=list(test.name="test",R=1,pls=FALSE,prune=TRUE,std=TRUE,
	...
	GetModel <- function(i) {		
		object<-glmboost(target~.,data.frame(target,train.data[,-1]),
	...
	MboostGlmboostPredict <- function(test.name="test",save.preds=TRUE) {
		...
		if(save.preds) {
			...
			out.file = GetPredictionFilename("MboostGlmboostTrain",train.name,test.name,args)
			WritePredictions(predictions, out.file)
		}
		return(predictions)
	}
	return(MboostGlmboostPredict)
}